...
export class ApplicationRestCredentialsComponent implements OnInit {
  ...
  hasAccessToVirtualBucket(virtualBucket: VirtualBucket) {
    return this.selectedAuth0Client.client_metadata.virtualBuckets.indexOf(virtualBucket.virtualBucketName) > -1;
  }

  updateVirtualBucketAccess(virtualBucket: VirtualBucket, $event) {
    if ($event.checked) {
      this.selectedAuth0Client.client_metadata.virtualBuckets += virtualBucket.virtualBucketName + ' ';
    } else {
      const startLoc = this.selectedAuth0Client.client_metadata.virtualBuckets.indexOf(virtualBucket.virtualBucketName);
      this.selectedAuth0Client.client_metadata.virtualBuckets =
        ...
        this.selectedAuth0Client.client_metadata.virtualBuckets.substring(startLoc + virtualBucket.virtualBucketName.length + 1);
    }
